...
async function createBookings() {
    ...
    const finalBookings = bookings.map((booking) => {
        ...
        let status;
        if (
            isPast(new Date(booking.ending_date)) &&
            !isToday(new Date(booking.ending_date))
        )
            status = "checked-out";
        if (
            isFuture(new Date(booking.starting_date)) ||
            isToday(new Date(booking.starting_date))
        )
            status = "unconfirmed";
        if (
            (isFuture(new Date(booking.ending_date)) ||
                isToday(new Date(booking.ending_date))) &&
            isPast(new Date(booking.starting_date)) &&
            !isToday(new Date(booking.starting_date))
        )
            status = "checked-in";
